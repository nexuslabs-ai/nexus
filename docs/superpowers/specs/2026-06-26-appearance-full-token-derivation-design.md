...
| Q2  | **Chart-categorical = fixed colorblind-safe set**, re-toned L/C per mode. Light chart series 2 uses `lime.600` instead of `lime.700` so lime/orange remain distinguishable under deuteranopia.                                                   | Categorical distinguishability is correctness; must pass `audit:colorblind`.                                                                                                             |
