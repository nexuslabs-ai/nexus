...
| Checkbox groups                       | Nexus supports checkbox atoms and Field composition, but #617 should inspect whether the row pattern is discoverable enough.                                                                                         | **Exported component direction.** Track compact checkbox and radio choice rows through `ChoiceRow` as the official row shell; keep Checkbox and Radio primitives unchanged.                                         | "Add exported ChoiceRow component for checkbox/radio row choices (#618)."   |
