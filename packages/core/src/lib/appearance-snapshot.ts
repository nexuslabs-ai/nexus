...
const RETIRED_STYLE_DATA_ATTR = `data-${'style'}`;

export const NEXUS_RETIRED_APPEARANCE_DATA_ATTRS = [
  RETIRED_STYLE_DATA_ATTR,
] as const;
...
export function createNexusAppearanceBootstrapScript(
  ...
  return `(function(){try{var k=${escapeInlineScriptJson(storageKey)};var f=${escapeInlineScriptJson(defaultSnapshot)};var s=f;try{var r=window.localStorage&&window.localStorage.getItem(k);if(r){var p=JSON.parse(r);if(p&&p.version===f.version&&p.state&&typeof p.themeCss==="string"&&typeof p.prefsCss==="string"){s=p;}}}catch(e){}var st=s.state||f.state;var m=st.mode==="dark"||st.mode==="light"||st.mode==="system"?st.mode:f.state.mode;var sys=false;try{sys=m==="system"&&window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches;}catch(e){}var dark=m==="dark"||sys===true;var root=document.documentElement;root.classList.toggle("dark",dark);root.removeAttribute(${escapeInlineScriptJson(NEXUS_RETIRED_APPEARANCE_DATA_ATTRS[0])});root.setAttribute(${escapeInlineScriptJson(NEXUS_APPEARANCE_DATA_ATTRS[0])},typeof st.density==="string"?st.density:f.state.density);root.setAttribute(${escapeInlineScriptJson(NEXUS_APPEARANCE_DATA_ATTRS[1])},typeof st.corners==="string"?st.corners:f.state.corners);root.setAttribute(${escapeInlineScriptJson(NEXUS_APPEARANCE_DATA_ATTRS[2])},typeof st.elevation==="string"?st.elevation:f.state.elevation);root.setAttribute(${escapeInlineScriptJson(NEXUS_APPEARANCE_DATA_ATTRS[3])},typeof st.stroke==="string"?st.stroke:f.state.stroke);root.style.colorScheme=dark?"dark":"light";var meta=document.querySelector('meta[name="color-scheme"]');if(!meta){meta=document.createElement("meta");meta.setAttribute("name","color-scheme");document.head.appendChild(meta);}meta.setAttribute("content",m==="system"?"light dark":m);function upsert(attr,css){var list=document.querySelectorAll("style["+attr+"]");var el=list[0]||document.createElement("style");for(var i=1;i<list.length;i++){list[i].remove();}el.setAttribute(attr,"");el.textContent=css||"";if(!el.parentNode){document.head.appendChild(el);}}upsert("data-nexus-appearance-theme",s.themeCss);upsert("data-nexus-appearance-prefs",s.prefsCss);}catch(e){}})();`;
}
