...
ruleTester.run('prefer-role-utilities', rule, {
  valid: [
    ...
    '// nexus-allow-numeric: chip rhythm\nconst x = `nx:px-2 nx:py-0.5 nx:gap-1`;',
    // Static template literals with role utilities are valid
    'const x = `nx:p-container nx:gap-control-md`;',
  ...
  invalid: [
    ...
    // Static template literals are checked
    {
      code: 'const x = `nx:p-4`;',
      errors: [{ messageId: 'preferRole' }],
    },
    ...
    {
      code: 'const X = <div className={`nx:gap-4`} />;',
      errors: [{ messageId: 'preferRole' }],
      languageOptions: {
        parser: tseslint.parser,
        parserOptions: { ecmaFeatures: { jsx: true } },
...
describe('prefer-role-utilities — bad-fixture smoke test', () => {
  it('reports seven findings on bad-prefer-role.tsx (one per raw-numeric utility)', async () => {
    const eslint = new ESLint({
      overrideConfigFile: true,
      overrideConfig: {
        files: ['**/*.tsx'],
    ...
    const code = readFileSync(filePath, 'utf8');
    const messages = await eslint.lintText(code, { filePath });
    // Lines 4 (p-4), 5 (px-2, py-2 — two findings), 6 (gap-4),
    // 7 (p-2.5), 8 (hover:p-4), 9 (`p-4`)
    expect(messages[0].errorCount).toBe(7);
