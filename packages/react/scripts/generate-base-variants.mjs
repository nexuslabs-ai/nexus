...
/**
 * Validate (or default) the config entry's `sourceDir`, throwing fast on a
 * typo or unsupported subdir so a downstream import-path/file-path lookup
 * never silently lands in the wrong directory. Resolved once per component
 * in `main()` and threaded into the path helpers below.
 */
function resolveSourceDir(component) {
...
/**
 * Relative TS import path from a generated story to its canonical stories
 * module. Exported so the unit test can assert on it without spawning the
 * full generator.
 */
...
function buildStory(component, config, sourceDir) {
  ...
  const storiesPath = storiesImportPath(component, sourceDir);
 ...
 */
function assertShowcaseExists(component, sourceDir) {
  const file = storiesFilePath(component, sourceDir);
...
function main() {
  ...
  for (const component of config.components) {
    const sourceDir = resolveSourceDir(component);
    assertShowcaseExists(component, sourceDir);
    const fileName = `${component.name}.base-variants.stories.tsx`;
    fs.writeFileSync(path.join(OUT_DIR, fileName), buildStory(component, config, sourceDir));
