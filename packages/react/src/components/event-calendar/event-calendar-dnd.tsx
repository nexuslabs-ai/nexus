 ...
 * overlays (carry clone, drop hint) are their own `nx:will-change: nx:transform`
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  let carryDY = 0;
  const CARRY_CLASS =
    'nx:bg-background nx:pointer-events-none nx:fixed nx:top-0 nx:left-0 nx:z-100 nx:overflow-hidden nx:rounded-sm nx:opacity-90 nx:shadow-md nx:will-change-transform' +
  ...
  const updateHint = (e: PointerEvent) => {
    ...
    if (!hintEl) {
      ...
      // in RTL documents
      hintEl.className =
        'nx:bg-background nx:text-error-foreground nx:border-border-error nx:pointer-events-none nx:fixed nx:top-0 nx:left-0 nx:z-100 nx:rounded-md nx:border nx:px-2 nx:py-0.5 nx:typography-label-small nx:shadow-sm' +
