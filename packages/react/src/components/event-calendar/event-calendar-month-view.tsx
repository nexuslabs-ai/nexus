...
/** The "+N more" trigger carries the drop-target event colour as a CSS var. */
type MoreIndicatorStyle = CSSProperties & { '--ec-event-color': string };

// Shared empty set for a column with no hidden bars, so the fallback never
// allocates a fresh Set per render.
const EMPTY_HIDDEN_BAR_KEYS: ReadonlySet<string> = new Set();
...
function EventCalendarMonthView({
  ...
  const weeks = useMemo(() => {
    ...
    const visible = effective.weekends
      ? rows
      : rows.map((row) =>
          row.filter(
            (day) =>
    ...
    // A rendered row always has a first day; narrowing here keeps every
    // downstream week[0] read total without a non-null assertion.
    return visible.filter((row): row is [Date, ...Date[]] => row.length > 0);
  }, [
    visibleRange,
    settings.timeZone,
    ...
    <EventCalendarViewContext.Provider value={{ view: 'month' }}>
      ...
      >
        <div
          ...
          // nx:@container scopes the narrow-label breakpoint to the header row
          className={cn(
            'nx:@container nx:grid nx:border-b',
        ...
        >
          ...
          {headerDays.map((day) => (
            <div
              ...
              role="columnheader"
              className={cn(
                'nx:text-muted-foreground nx:truncate nx:px-2 nx:py-1.5 nx:typography-label-default',
                viewConfig.classNames?.monthDayHeader
              )}
            >
              {
                <>
                  <span className="nx:@max-[36rem]:hidden">
                    {format(
                  ...
                  <span className="nx:hidden nx:@max-[36rem]:inline">
                    {format(
                      toZoned(day, settings.timeZone),
        ...
        <div
          ...
          className={cn(
            'nx:grid',
            contained && 'nx:min-h-0 nx:flex-1',
        ...
        {autoFit && (
          <div
            ref={slotProbeRef}
            aria-hidden
            className="nx:pointer-events-none nx:invisible nx:absolute nx:h-[var(--ec-month-bar-h,1.75rem)] nx:w-0"
          />
        )}
      </Comp>
...
function EventCalendarMonthWeek({
  ...
  const { bars, rowStart } = useEventCalendarWeek(weekStart);
  ...
  const hiddenBarKeysByCol = week.map(
    (_, col) =>
      new Set(
        bars
          .filter((b) => (b.lane ?? 0) >= cap && covers(b, offsets[col] ?? 0))
  ...
  return (
    <div
      ...
      className={cn(
        'nx:relative nx:grid nx:min-h-0 nx:border-b nx:last:border-b-0',
    ...
    >
      {showWeekNumber && (
        <div
          ...
          className={cn(
            'nx:text-muted-foreground nx:border-e nx:px-2 nx:pt-1 nx:tabular-nums',
            viewConfig.classNames?.weekNumber
          )}
        >
          {settings.i18n.labels.week(
            getWeek(toZoned(weekStart, settings.timeZone), {
      ...
      {week.map((day, col) => (
        <EventCalendarMonthCell
          ...
          reservedLanes={visibleBars.reduce(
            (max, b) =>
              covers(b, offsets[col] ?? 0)
                ? Math.max(max, (b.lane ?? 0) + 1)
                : max,
            0
          )}
          hiddenBarKeys={hiddenBarKeysByCol[col] ?? EMPTY_HIDDEN_BAR_KEYS}
      ...
      {(visibleBars.length > 0 || (dragGhost && ghostPos && ghostIsBar)) && (
        <div
          ...
          className={cn(
            'nx:pointer-events-none nx:absolute nx:inset-x-0 nx:top-0 nx:z-10 nx:grid nx:pt-1.5',
          ...
          {visibleBars.map((bar) => {
            ...
            return (
              <div
                ...
                role="gridcell"
                aria-colspan={pos.span}
                className={cn(
                  'nx:pointer-events-auto nx:min-w-0 nx:px-1',
              ...
              >
                ...
                <EventCalendarEvent
                  segment={bar}
                  className="nx:h-[calc(var(--ec-month-bar-h,1.75rem)-0.125rem)]"
                />
              </div>
            );
          })}
          {dragGhost && ghostPos && ghostIsBar && (
            ...
            >
              <div
                ...
                className={cn(
                  'nx:h-[calc(var(--ec-month-bar-h,1.75rem)-0.125rem)]',
                  dragGhost.kind === 'move'
...
function EventCalendarMonthCell({
  ...
  hiddenBarKeys: ReadonlySet<string>;
  ...
  const dropPlaceholder = inlineDrop ? (
    <div
      ...
      className={cn(
        'nx:shrink-0 nx:rounded-sm nx:border nx:border-dashed',
        inlineDrop.valid
          ? 'nx:border-(--ec-event-color)/50 nx:bg-(--ec-event-color)/8'
          : 'nx:border-border-error nx:bg-error-subtle',
        viewConfig.classNames?.dragGhost
      )}
      style={
        {
          '--ec-event-color':
            inlineDrop.color ?? 'var(--color-chart-categorical-1)',
    ...
    <>
      <div
        ...
        className={cn(
          ...
          'nx:flex nx:min-h-0 nx:flex-1 nx:flex-col nx:gap-0.5 nx:overflow-hidden nx:px-1 nx:pt-1.5',
      ...
      <div
        className={cn(
          'nx:flex nx:items-center nx:justify-end nx:gap-1 nx:px-2 nx:pb-1.5',
      ...
      >
        {viewConfig.showDayAddButton && !isInteracting && (
          <button
            ...
            className={cn(
              'nx:bg-primary-background nx:text-primary-foreground nx:flex nx:size-5 nx:cursor-pointer nx:items-center nx:justify-center nx:rounded-sm nx:opacity-0 nx:transition-opacity nx:group-hover/ec-cell:opacity-100 nx:focus-visible:opacity-100',
          ...
          >
            <IconPlus className="nx:size-3.5" aria-hidden="true" />
          </button>
        )}
        <span
          data-slot="event-calendar-month-day-number"
          className={cn(
            'nx:flex nx:size-5 nx:items-center nx:justify-center nx:rounded-full',
            isOutside && 'nx:text-muted-foreground',
            ...
            isToday && 'nx:bg-primary-background nx:text-primary-foreground',
  ...
  return (
    <div
      ...
      className={cn(
        'nx:group/ec-cell nx:relative nx:flex nx:min-h-0 nx:min-w-0 nx:flex-col nx:overflow-hidden',
        !isLast && 'nx:border-e',
        isOutside && !settings.showOutsideDays && 'nx:invisible',
        isOff && offClassName,
        isToday &&
          cn(
            'nx:bg-primary-subtle nx:border-b-border-primary nx:relative nx:border-b-2',
    ...
    >
      {inDraft && (
        <span
          ...
          className={cn(
            'nx:border-border-primary nx:pointer-events-none nx:absolute nx:inset-0 nx:z-10 nx:border-y nx:border-dashed',
            inDraft.isStart && 'nx:border-s',
            inDraft.isEnd && 'nx:border-e',
...
function EventCalendarMoreIndicator({
  ...
  const dropIntoStyle: MoreIndicatorStyle = {
    '--ec-event-color': dropInto?.color ?? 'var(--color-chart-categorical-1)',
    height: 'calc(var(--ec-month-bar-h, 1.75rem) - 0.125rem)',
  };
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        ...
        className={cn(
          'nx:text-muted-foreground nx:hover:text-foreground nx:cursor-pointer nx:truncate nx:rounded-sm nx:px-1.5 nx:text-start',
          ...
          dropInto &&
            cn(
              'nx:flex nx:shrink-0 nx:items-center nx:border nx:border-dashed',
              dropInto.valid
                ? 'nx:text-foreground nx:border-(--ec-event-color)/50 nx:bg-(--ec-event-color)/8'
                : 'nx:border-border-error nx:bg-error-subtle nx:text-error-foreground'
            ),
          viewConfig.classNames?.moreIndicator
        )}
        style={dropInto ? dropIntoStyle : undefined}
      ...
      <PopoverContent
        ...
        // The popover is a dialog, so it needs a name. The body renders the day
        // header this list belongs to, so point at that.
        ...
        className={cn(
          'nx:w-64 nx:gap-1! nx:p-2! nx:typography-body-small',
  ...
  return (
    <>
      <div
        id={headerId}
        className={cn(
          'nx:text-muted-foreground nx:px-1 nx:py-1 nx:typography-label-small',
      ...
      {viewConfig.scrollbars === 'native' ? (
        <div
          ...
          className="nx:-me-2 nx:max-h-(--ec-more-max-height,16rem) nx:min-h-0 nx:overflow-y-auto"
        >
          <div className="nx:flex nx:flex-col nx:gap-1 nx:py-1 nx:ps-1 nx:pe-4">
        ...
        <ScrollArea className="nx:-me-2 nx:min-h-0 nx:**:data-[slot=scroll-area-viewport]:max-h-(--ec-more-max-height,16rem)">
          <div className="nx:flex nx:flex-col nx:gap-1 nx:py-1 nx:ps-1 nx:pe-4">
            {segments.map((segment) => (
              <EventCalendarEvent
                key={segment.occurrence.key}
