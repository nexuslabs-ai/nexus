 ...
 */
interface EventCalendarViewConfig {
  scrollToHour: number;
...
const EventCalendarViewConfigContext =
  createContext<EventCalendarViewConfig>(DEFAULT_VIEW_CONFIG);

/** Root-level display props, for view components. */
function useEventCalendarViewConfig(): EventCalendarViewConfig {
...
function splitOptions<TData>(props: Record<string, unknown>): {
  ...
  return {
    ...
    viewConfig: viewConfig as unknown as EventCalendarViewConfig,
    rest,
  };
}
    ...
    <EventCalendarContext.Provider value={instance}>
      <EventCalendarViewConfigContext.Provider value={viewConfig}>
        <Comp
          ...
          // e.g. <EventCalendar className="nx:typography-body-small"> and every inheriting
          ...
          className={cn(
            'nx:flex nx:min-h-0 nx:min-w-0 nx:flex-col nx:typography-body-small',
            className
          )}
          {...rest}
        >
          {/* Slottable keeps the announcer sibling legal when asChild slots
              the consumer element (Slot allows one non-Slottable child). */}
