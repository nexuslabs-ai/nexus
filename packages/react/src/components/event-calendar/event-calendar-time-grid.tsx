    ...
    <EventCalendarViewContext.Provider value={{ view }}>
      ...
      >
        ...
        >
          ...
          >
            {days.map((day) => (
              <EventCalendarDayHeader key={day.getTime()} day={day} />
            ))}
          </div>
        </div>
        {/* All-day row */}
...
function EventCalendarDayHeader({ day }: { day: Date }) {
  const settings = useEventCalendarSettings();
  const viewConfig = useEventCalendarViewConfig();
  ...
  return (
    <div
      ...
      className={cn(
        'nx:data-today:text-primary-subtle-foreground nx:min-w-0 nx:truncate nx:border-e nx:px-2 nx:py-1.5 nx:typography-label-default nx:last:border-e-0',
  ...
  >(
    () => {
      ...
      for (const bar of packed) {
        ...
        let row: boolean[] = [];
        for (;;) {
          row = lanes[lane] ??= new Array(days.length).fill(false);
          let free = true;
...
function EventCalendarAllDayBars({
  ...
  >(
    (state) => {
      ...
      const startDay = days[colStart];
      const endDay = days[colEnd];
      ...
      return {
        ...
        isStart: zonedStartOfDay(startDay, tz).getTime() <= startMs,
        isEnd: zonedStartOfDay(endDay, tz).getTime() >= lastMs,
  ...
  const ghostDay = dragGhost ? days[dragGhost.colStart] : undefined;

  return (
    <div className="nx:relative nx:min-w-0 nx:flex-1">
      <div
        className="nx:grid nx:h-full"
        style={{
          gridTemplateColumns,
      ...
      {(bars.length > 0 || dragGhost) && (
        <div
          ...
          className="nx:pointer-events-none nx:absolute nx:inset-x-0 nx:top-0 nx:grid nx:pt-1.5"
          style={{
            gridTemplateColumns,
          ...
          {dragGhost && ghostDay && (
            ...
            >
              <div
                ...
                className={cn(
                  'nx:h-[calc(var(--ec-month-bar-h,1.625rem)-0.125rem)]',
                  dragGhost.kind === 'move'
                    ? cn(
...
function EventCalendarAllDayCell({ day }: { day: Date }) {
  ...
  return (
    <div
      ...
      className={cn(
        'nx:relative nx:flex nx:min-w-0 nx:flex-col nx:gap-0.5 nx:border-e nx:px-1 nx:py-1.5 nx:last:border-e-0',
        isOff && offClassName,
...
function EventCalendarTimeGutter({
  ...
  return (
    <div
      ...
      className={cn(
        'nx:relative nx:w-(--ec-gutter-width,4.5rem) nx:shrink-0 nx:border-e',
    ...
    >
      {slots.map((minutes) => {
        ...
        // The label is suppressed at the day-start edge.
        const label =
        ...
        return (
          ...
          >
            {label != null && (
              <span
                className={cn(
                  'nx:text-muted-foreground nx:absolute nx:end-2.5 nx:-top-2',
...
function EventCalendarDayColumn({
  ...
  const packedTimed = useMemo(() => {
    ...
    const clones = visible.map(
      (segment): EventCalendarSegment => ({ ...segment })
    );
...
function EventCalendarNowIndicator({
  ...
  return (
    <div
      ...
      className="nx:pointer-events-none nx:absolute nx:inset-x-0 nx:z-40"
      style={{ top: `calc(var(--ec-hour-height) * ${top})` }}
    >
      {/* hairline across the content columns only (clear of the time gutter) */}
      <div className="nx:bg-error-background nx:absolute nx:start-(--ec-gutter-width,4.5rem) nx:end-0 nx:h-px" />
      {/* stronger segment + dot over today's column */}
      <div
        className="nx:absolute nx:h-px"
        style={{
      ...
      >
        <div className="nx:bg-error-background nx:absolute nx:inset-x-0 nx:top-0 nx:h-px" />
        ...
        <div className="nx:bg-error-background nx:absolute nx:-start-1 nx:top-0 nx:size-1.5 nx:-translate-y-1/2 nx:rounded-full" />
      </div>
    </div>
  );
}
