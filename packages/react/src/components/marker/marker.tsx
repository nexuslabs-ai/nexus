...
const markerVariants = cva(
  "nx:group/marker nx:flex nx:min-h-4 nx:w-full nx:items-center nx:gap-2 nx:text-left nx:typography-body-small nx:text-muted-foreground nx:[&_svg:not([class*='size-'])]:size-4 nx:[&_svg]:shrink-0 nx:[&_a]:underline nx:[&_a]:underline-offset-3 nx:[&_a:hover]:text-foreground",
  {
    variants: {
      variant: {
        default: '',
        separator:
          'nx:before:mr-1 nx:before:h-px nx:before:min-w-0 nx:before:flex-1 nx:before:bg-border-default nx:after:ml-1 nx:after:h-px nx:after:min-w-0 nx:after:flex-1 nx:after:bg-border-default',
...
interface MarkerProps
   ...
   *
   * Marker cannot know whether the child is interactive, so it applies no
   * hover treatment to the row itself. Add `nx:hover:text-foreground` when the
   * child is a link or button.
   *
   * @default false
   * @example
   * ```tsx
 ...
 * MarkerContent
 *
 * The Marker's label. Under `variant="separator"` it stops growing so the
 * rules take the remaining space and the label centres between them.
 */
function MarkerContent({ className, ...props }: MarkerContentProps) {
  return (
    <span
      ...
      className={cn(
        'nx:min-w-0 nx:wrap-break-word nx:group-data-[variant=separator]/marker:flex-none nx:group-data-[variant=separator]/marker:text-center',
        className
      )}
      {...props}
    />
