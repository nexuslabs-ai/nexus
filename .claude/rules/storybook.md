...
```json
{ "name": "Button", "storiesFile": "Button.stories", "showcase": "AllVariants" }
```

- `name` — the component (also the sidebar title and output filename prefix).
- `storiesFile` — the canonical stories module under `../ui/`, without extension.
- `showcase` — a **render-based** export from that module (e.g. `AllVariants`; Avatar uses `AllSizes`). The generator reuses this story's `render()` directly, so it must not be args-only.
...
| `yarn test` / `yarn test:storybook`       | regenerate, then run vitest (addon-vitest sees them) |

The output is gitignored, so a fresh checkout has no generated files until one of the above runs.
