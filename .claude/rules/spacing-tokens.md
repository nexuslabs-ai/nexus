...
**Cross-mode visual consequence.** Because `padding-x` is mode-stable while Button's `padding-x` is mode-coupled, the visual gap between an Input and an adjacent Button varies by mode: `--control-padding-x-md` is 12px (nova) → 16px (vega/lyra/luma/mira) → 20px (maia) → 24px (sera). At Input's fixed 12px that lands as: **flush in nova (0px gap), ~4px in vega, ~8px in maia, ~12px in sera per side.** The "narrower by design" intent therefore only holds at certain densities — at compact modes the two controls visually merge. The same shape will be inherited by Select trigger in batch 2 of #124 (and any future `combobox` / `textbox`-tier control that lands on this convention). Before batch 2 merges, the design call should be made explicitly: either accept the per-mode gap drift as intentional (i.e. compact modes _should_ render form fields flush with buttons), retune the role-token vega values so `--control-padding-x-md` matches Input's 12px and re-derive the other modes, or introduce an `--input-padding-x-*` family. All three are larger conversations than this migration — flagged here so the decision surfaces before Select inherits it.
