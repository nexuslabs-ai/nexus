...
Not every surface takes this ring — see [§ Surface exception map](#surface-exception-map) for which surface types use the ring, a background-tint `:focus`, or no focus treatment at all.
...
Not every focusable thing takes the outline ring, and not everything that shows a `:focus` state is a keyboard control. Which treatment a surface gets is decided by its **input modality**, not its visual elevation:

| Surface type                                                                                                                                     | Pattern                                                                                                                                   | Rationale                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **Menu items** — DropdownMenuItem, SelectItem, and the DropdownMenu sub-trigger / checkbox / radio items (any Radix primitive with roving focus) | `nx:focus:bg-background-hover` — background tint, **no ring**                                                                             | Radix roving focus moves DOM focus to the item under the pointer, so `:focus` fires on mouse-hover too. A `:focus-visible` ring would flash for pointer users while giving them no steady indicator — wrong UX. A background tint reads correctly for both keyboard and pointer. |
...
The dividing line is modality: anything a keyboard user reaches with Tab gets the ring; anything Radix's roving focus moves to on pointer-hover gets the background tint. The tint pattern is live in `dropdown-menu.tsx` (item, sub-trigger, checkbox, and radio items) and `select.tsx` (item).
