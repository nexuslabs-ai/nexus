...
| `Marker` annotation row       | `nx:min-h-4`; single size, no size prop; `variant="border"` adds `nx:border-b-default` + `nx:pb-2`; `variant="separator"` rules are `nx:h-px` `::before` / `::after` flex items; when `asChild` makes the row an `a` or `button` it takes `nx:pointer-coarse:min-h-11`, so the interactive row clears the ~44px tap floor | `nx:gap-2`; no padding of its own; separator rules add `nx:before:mr-1` / `nx:after:ml-1` on top of the row gap; nested `MarkerIcon` is `nx:size-4` and unwrapped `svg` children are sized to match | `variant="separator"` is `nx:w-full` so the rules have width to expand into; `default` and `border` take their parent's layout width; `MarkerContent` is `nx:min-w-0` and centres its text under `variant="separator"` while the rules take the slack |
