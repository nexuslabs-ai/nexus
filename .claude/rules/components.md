...
| **Menu items** — DropdownMenuItem, SelectItem, and the DropdownMenu sub-trigger / checkbox / radio items (any Radix primitive with roving focus) | `nx:focus:bg-background-hover nx:focus:text-foreground` — background tint, **no ring**                                                    | Radix roving focus moves DOM focus to the item under the pointer, so `:focus` fires on mouse-hover too. A `:focus-visible` ring would flash for pointer users while giving them no steady indicator — wrong UX. A background tint reads correctly for both keyboard and pointer. |
