...
| `Attachment` horizontal row   | Natural height; `default`: `nx:p-2.5`, `sm`: `nx:p-2`; `nx:rounded-md`; `state="idle"` swaps the `Item` outline border to dashed, `state="error"` to `nx:border-border-error` | `default`: `nx:gap-2.5`, `sm`: `nx:gap-2`; nested `ItemMedia` keeps its own atom sizing           | Layout-controlled; `nx:flex-none` when inside `AttachmentGroup`                 |
| `Attachment` vertical tile    | Fixed `nx:w-32`; natural height; nested `ItemMedia` overridden to `nx:w-full` at `nx:aspect-square` so the thumbnail fills the card                                           | Same padding and gap as the horizontal row at the active size                                     | Fixed `nx:w-32` — deliberately not layout-controlled, so strips stay even       |
