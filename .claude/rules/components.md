...
| `ChoiceCard` option card      | Natural height; `nx:p-4`; `variant="outline"` uses the card border; `floating=true` rounds with `nx:rounded-md`, `floating=false` uses attached edges | `nx:gap-3`; `controlPosition="before/after"` keeps the nested marker atom-sized        | Layout-controlled `nx:w-full`; nested marker remains atom-sized               |
