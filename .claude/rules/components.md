...
| `AttachmentProgress` bar      | `nx:h-1`, deliberately slimmer than `Progress`'s shipped `nx:h-2` — it sits inside `ItemContent` under the description as a secondary signal, not a primary control                                         | `nx:mt-1` off the description; no inline padding                                                  | Layout-controlled `nx:w-full`, inherited from `Progress`                                               |
