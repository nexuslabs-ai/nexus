...
| `Bubble` conversational turn  | Natural height; `nx:rounded-xl`; single size, no size prop; every variant reserves `nx:border-default nx:border-transparent` so `variant="outline"` adds no layout shift; the surface deliberately does not clip, so `BubbleReactions` can overhang it | `nx:px-4`, `nx:py-3`; `BubbleContent` adds no padding of its own                                                                              | `nx:w-fit nx:max-w-[min(80%,45rem)]` — the `80%` keeps an alignment gutter that scales with the conversation column so `align` stays legible at every width, and the `45rem` cap holds the reading measure when the column is wide; `align` is auto inline margins (`nx:me-auto` / `nx:ms-auto`), never a physical side |
| `BubbleGroup` turn stack      | Natural height                                                                                                                                                                                                                                         | `nx:gap-4` — load-bearing, not decorative: it is the clearance that keeps an overhanging `BubbleReactions` pill off the next bubble's surface | Layout-controlled `nx:w-full`; children carry their own `align`                                                                                                                                                                                                                                                         |
| `BubbleReactions` pill        | `nx:rounded-full`; absolutely positioned; `side="top"` / `side="bottom"` overhang the matching edge by `nx:-top-3` / `nx:-bottom-3`, kept clear of the neighbouring turn by `BubbleGroup`'s `nx:gap-4`                                                 | `nx:px-2`, `nx:py-0.5`, `nx:gap-1`                                                                                                            | `nx:w-fit`; `align="start"` / `align="end"` inset the pill 12px from the inline edge via `nx:start-3` / `nx:end-3`, which are logical — `start` resolves to the right under `dir="rtl"`                                                                                                                                 |
