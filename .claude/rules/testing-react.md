...
in `packages/react/scripts/storybook-coverage.config.json` — e.g. Avatar's showcase is
...
fields in `packages/react/scripts/storybook-coverage.config.json` are the runtime
...
| Axis toggle         | Show, Hide   | Showcase story       | `AllAxes` (configured in `storybook-coverage.config.json`) |
