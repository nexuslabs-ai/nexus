...
This project is **pre-production** — there is no live deployment or real user data. Published tokens are a public API that other projects may consume.

## What This Means

- **Published token APIs.** Coordinate token renames with a tested consumer migration and a documented breaking release. Do not assume consuming projects update when Nexus changes. Temporary compatibility aliases require an explicit decision; do not add them by default.
