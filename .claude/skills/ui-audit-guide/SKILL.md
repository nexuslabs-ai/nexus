...
Before testing, read available design references: `.claude/rules/` (especially `components.md`, `shadcn-divergences.md`), the token docs in `packages/core/docs/` (`tokens.md`, `color-shades.md`, `surfaces.md`), and design tokens in `packages/core/tokens/`. Skip any that don't apply.
