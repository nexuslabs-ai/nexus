...
- DatePicker range days: `rounded-md` at the range start and end,
  `rounded-none` in the middle. A selected day at a week edge also takes
  `rounded-l-md` or `rounded-r-md` on that side. A single selected day, all
  other days and the month navigation buttons follow the override.
