...
- [x] `@nexus_ds/tailwind` and `@nexus_ds/react/styles.css` are both imported
- [x] The provider wraps your tree, and the script runs in `<head>`
- [ ] Reloading in dark mode produces no flash of the light theme[^flash]
- [ ] Toggling the mode persists across a full page reload

[^flash]: A flash means the script ran too late — it has to paint before the
first frame, not from a client effect.
