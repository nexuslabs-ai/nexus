...
This page continues from [Install](/getting-started/install): the token CSS is
copied into `styles/nexus/` and imported from your CSS entry point, the
appearance provider is copied into `components/appearance/`, and
`@nexus_ds/core` is installed. Every snippet below is a change to the
`app/layout.tsx` that page ends with, not a second file.
