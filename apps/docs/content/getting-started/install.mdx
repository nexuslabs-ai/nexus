...
Setup is done. A component is never just its own file: it imports other Nexus
files, and those import packages. Each component lists both, generated from its
imports. For Button, install these and copy these files first:

<InstallBlock slug="button" />

Then copy `components/button/button.tsx` itself. Every path is relative to
`packages/react/src/` in the repo: keep it as it is under your `@/` root, next
to the `lib/utils.ts` from step 4, and the relative imports between the files —
barrels included — resolve without an edit. Rerunning an install line or
recopying `lib/utils.ts` changes nothing.
