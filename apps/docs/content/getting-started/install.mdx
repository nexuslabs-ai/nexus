...
Import the entry point from your app's CSS entry point, followed by the
animation utilities some components use for their enter and exit states:
...
`nexus.css` is the whole token surface: the `nx:` utilities, the semantic color
...
<ComponentSource id="getting-started/button-core-variants" />

<ComponentPreview id="getting-started/button-core-variants" />
...
- **[Create](/create)** — style real Nexus components live, then copy the
  appearance into your project.
