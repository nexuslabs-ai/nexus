/**
 * The docs Content-Security-Policy: the header `next.config.ts` sends, and the
 * CSP Level 3 primitives needed to read one back. `scripts/csp-audit.mjs` builds
 * the audit's verdicts on top of these. `CSP.md` records what the policy allows
 * today and what still has to change before it can be enforced.
 */
