...
describe('createContentSecurityPolicy', () => {
  it('builds the exact header the docs app ships', () => {
    expect(SHIPPED_HEADER).toBe(
      "default-src 'self'; " +
        `script-src 'self' ${APPEARANCE_HASH} 'report-sample'; ` +
...
describe('parseContentSecurityPolicy prototype keys', () => {
  ...
  it('does not hand an inherited value back from resolveDirective', () => {
    expect(resolveDirective({}, ['constructor'])).toBeNull();
  });
});
