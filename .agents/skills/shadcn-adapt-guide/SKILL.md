...
- `yarn lint` — the canonical gate (`eslint . --max-warnings 0`), and it passes on a current tree (`.Codex/worktrees/` is config-ignored, so `eslint .` doesn't recurse it). For a faster loop while iterating, scope to your files: `npx eslint packages/react/src/components/ui/{name}.tsx`.
