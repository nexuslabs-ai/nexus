...
- _Authoring or restyling a component, or writing any `nx:` utility?_
  -> `nx:` prefix on every utility, **before** every modifier (`nx:hover:`, `nx:[&>svg]:`, `nx:lg:` — never `hover:nx:`). Semantic tokens only, full paths (`nx:bg-primary-background` — never `nx:bg-primary`, never a primitive like `nx:bg-blue-500`); no `dark:` on a semantic token (it already adapts — the modifier is a no-op). Size controls by copying the `size` variants in `button.tsx` / `input.tsx`; never a `control-*` spacing token (dropped in #489). `@container` for component-internal responsive; viewport prefixes (`nx:lg:`) are page-shell only (Dialog is the one viewport-driven exception). Focus = a real outline: controls `nx:focus-visible:outline-2 nx:focus-visible:outline-focus-default`, Button adds `nx:focus-visible:outline-offset-2`, fields use `nx:focus-visible:outline-default` plus `nx:focus-visible:border-focus-default` because the border is the ring's inner half and both halves read one borderwidth token. Never pair `nx:outline-none` with a focus-outline class on one element — it blanks the ring. Never put `nx:transition-colors` on an element that paints a ring — it includes `outline-color`, so it fades the ring in instead of landing it with the keypress (`pnpm lint` fails the pair). Use the ring-safe pair: `nx:transition-control` (color, background-color, border-color) for controls and Buttons, `nx:transition-field` (color, background-color) for field surfaces, whose border is the ring's inner half. Never use `nx:ring-*`. `data-slot` always; `data-variant` / `data-size` when the prop exists. CVA for enum variants; boolean props as ternaries in the body, not CVA. `asChild` via Radix `Slot` for interactive components. (`responsive.md`)
